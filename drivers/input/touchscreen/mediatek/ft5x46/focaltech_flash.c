...
static unsigned char CTPM_FW[] = {
	#include "./include/firmware1/IT_CS0x57_13877__V12_D01_20160106_app.h"
};
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	*/
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x54))
	{
		...
		if (i_ret != 0)
		{
			printk(KERN_ERR "[FTS] upgrade failed. err=%d.\n", i_ret);
		}
		else 
		{
			#ifdef AUTO_CLB
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	{
		msleep(100);
		printk(KERN_ERR "[FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",uc_tp_fm_ver, uc_host_fm_ver);
		i_ret = fts_ctpm_fw_upgrade_with_i_file(client);
		if (i_ret == 0)	
		{
			msleep(300);
			uc_host_fm_ver = fts_ctpm_get_i_file_ver();
			printk(KERN_ERR  "[FTS] upgrade to new version 0x%x\n",uc_host_fm_ver);
		} 
		else
		{
